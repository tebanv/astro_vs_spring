<script>
    ...
    import ListDictionaries from './ListDictionaries.svelte';
    ...
    function addDictionary() {
      if ((activeTab === 'names' || activeTab === 'codes') && selectedColumn && selectedDictionary && columnNameInDictionary) {
        rulesStore.update(store => {
        ...
        columnNameInDictionary = '';
      } else if (activeTab === 'habits' && columnNameHabit && columnNameToCompare && habits.length > 0 && selectedDictionary && columnNameInDictionary) {
        rulesStore.update(store => {
      ...
      <div class="mt-4 space-y-4">
        ...
        {#if activeTab === 'names' || activeTab === 'codes'}
          <div class="space-y-2">
            ...
            <ListDictionaries bind:selectedDictionary />
            ...
            <button 
              ...
              disabled={!selectedColumn || !selectedDictionary || !columnNameInDictionary}
        ...
        {:else if activeTab === 'habits'}
          <div class="space-y-2">
            ...
            >
              <option value="">Seleccionar columna para hábito</option>
              {#each headers as header}
                <option value={header}>{header}</option>
              {/each}
            </select>
            ...
            <button 
              on:click={addDictionary}
              disabled={!columnNameHabit || !columnNameToCompare || habits.length === 0 || !selectedDictionary || !columnNameInDictionary}
              class="w-full bg-green-500 text-white p-2 rounded-md hover:bg-green-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
